      ...
      <div class="main-container tech-theme user-setup-main" :class="{'light-theme': !isDarkTheme}">
        <div class="user-setup-grid">
          ...
          <div class="user-content-card">
            <div class="user-content-title">流水信息</div>
            <div class="user-history-list">
              <div v-if="total === 0" class="no_contet no_record">您还没有使用或获取过书币哦！</div>
              <div v-else>
                <div class="dataTable">
                  <table>
                    <thead>
                    ...
                    <tbody>
                      <tr v-for="(log, index) in walletLogs" :key="log.id || `empty-${index}`">
                        ...
                        <td v-if="log.empty"></td>
                        <td v-else class="record-desc">{{ log.subject }}</td>
                        <td v-if="log.empty"></td>
                        <td v-else :class="{'text-red': log.amount.startsWith('-'),'text-green': !log.amount.startsWith('-'), 'record-amount': true}">
...
.page-wrapper:not(.light-theme) .record-desc {
  color: #fff;
}
.record-amount {
  color: #333;
}
.page-wrapper:not(.light-theme) .record-amount {
  color: #fff;
}
.text-red {
  color: red;
}
.page-wrapper:not(.light-theme) .text-red {
  color: #ff7676;
}
.text-green {
  color: green;
}
.page-wrapper:not(.light-theme) .text-green {
  color: #7fff7f;
}
.el-pagination {
